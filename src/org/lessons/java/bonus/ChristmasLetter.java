...
//          se il booleano è false il metodo si chiude sollevando un’eccezione.
    ...
    public Map<String, Integer> contaDesideri(){
        Map<String, Integer> mappaDesideri = new HashMap<>();
        for (int i = 0; i < wishes.size(); i++) {
            if (mappaDesideri.containsKey(wishes.get(i))){
                mappaDesideri.put(wishes.get(i), (mappaDesideri.get(wishes.get(i)) +1));
            } else {
                mappaDesideri.put(wishes.get(i), 1);
            }
        }
